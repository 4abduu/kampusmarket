...
// ── Helper: extract data from a single Order ──
...
export default function PaymentSuccessPage({
      ...
      try {
        ...
        if (storedIds) {
          try {
            ...
            // Don't remove yet — handle StrictMode double mount
          } catch {
            console.error("[PaymentSuccessPage] Failed to parse recentCheckoutOrderIds");
          }
        }
        ...
        const results = await Promise.all(
          orderIds.map((id) =>
            getOrderDetail(id).catch((err) => {
              console.error(`[PaymentSuccessPage] Failed to fetch order ${id}`, err);
              return null;
            })
          )
        );
        ...
        console.error("[PaymentSuccessPage] Failed to load orders", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const grandTotal = orderDataList.reduce((sum, o) => sum + o.total, 0);

  const getNextSteps = () => {
    const method = firstOrderData?.shippingMethod || "COD";
    switch (method) {
      case "COD":
        return [
      ...
      <div className="container mx-auto px-4 py-8 max-w-2xl">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">
            {isMultiOrder
              ? "Semua pesananmu sedang diproses. Penjual akan segera menghubungimu."
            ...
            <Card className="mb-6">
              <CardContent className="p-6">
                <h2 className="font-semibold mb-4 flex items-center gap-2">
                  ...
                  Daftar Pesanan ({orderDataList.length})
                </h2>

                <div className="space-y-4">
                  {orderDataList.map((od, idx) => (
                    <div key={od.id || idx}>
                      <div className="flex items-center gap-4">
                        <div className="w-14 h-14 bg-slate-100 dark:bg-slate-800 rounded-lg flex items-center justify-center shrink-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {od.product.quantity}x •{" "}
                            {formatPrice(od.product.price)}
                          </p>
                          <p className="text-xs text-muted-foreground">
                            Penjual: {od.product.seller.name}
                          </p>
                ...
                <div className="mt-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-800">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium">
                      Metode Pengiriman: {firstOrderData?.shippingMethod}
                    </span>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="p-6">
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                      Penjual: {firstOrderData?.product.seller.name}
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-4 p-3 rounded-lg bg-slate-50 dark:bg-slate-800">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Metode Pembayaran:{" "}
                    {firstOrderData?.paymentMethod ||
                      firstOrderData?.shippingMethod}
                  </p>
                  {firstOrderData?.shippingMethod === "COD" && (
                    <p className="text-xs text-muted-foreground mt-1">
        ...
        <div className="space-y-3">
          {isMultiOrder ? (
            // Multi-order: go to order list instead of single detail
            <Button
            ...
            // Single-order: go to detail of that specific order
            <Button
              className="w-full bg-primary-600 hover:bg-primary-700"
              size="lg"
              onClick={() =>
