...
export default function ProfilePage({ onNavigate, userId }: ProfilePageProps) {
  ...
  const [productPriceRange, setProductPriceRange] = useState<number[]>([0, 1000000]);
  const [productSortBy, setProductSortBy] = useState<ProductSortBy>("terbaru");

  const [serviceCategory, setServiceCategory] = useState<string | null>(null);
  const [servicePriceRange, setServicePriceRange] = useState<number[]>([0, 1000000]);
  ...
  const filteredProducts = useMemo(() => {
    const filtered = [...userProducts]
      ...
      .filter((p) => p.price >= productPriceRange[0] && p.price <= productPriceRange[1]);
  ...
  const filteredServices = useMemo(() => {
    const filtered = [...userServices]
      .filter((service) => {
        ...
        return catId === serviceCategory || service.category.toLowerCase() === serviceCategory.toLowerCase();
      })
      .filter((service) => {
        const minPrice = "priceMin" in service ? service.priceMin ?? 0 : service.price ?? 0;
          ...
          <div className="lg:col-span-2 space-y-4">
            <Tabs value={activeTab} onValueChange={handleTabChange}>
              ...
              <TabsContent value="reviews" className="mt-4">
                <ProfileReviewsTab avgRating={avgRating} totalReviews={totalReviews} />
              </TabsContent>
            </Tabs>
          </div>
        </div>
