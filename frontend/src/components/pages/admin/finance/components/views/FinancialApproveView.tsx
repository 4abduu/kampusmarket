...
import { FinancialUserCard, FinancialAmountCard } from "../shared";
import { Button } from "@/components/ui/button";
import { CheckCircle2, ChevronLeft, Landmark } from "lucide-react";

export const FinancialApproveView: React.FC = () => {
  const { withdrawal, setVariant, onApprove, loading } = useFinancialModal();

  if (!withdrawal) return null;

  return (
    <div className="space-y-3.5 py-1 text-center flex flex-col items-center">
      {/* Figma centang green circle indicator */}
      ...
      <div className="space-y-0.5">
        <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">
          Setujui Penarikan Dana
        </h3>
        <p className="text-xs text-muted-foreground max-w-[320px] mx-auto leading-relaxed">
          Status transaksi akan diubah menjadi <strong className="text-blue-600 font-semibold">Diproses</strong>.
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2 pt-2 border-t border-slate-100 dark:border-slate-800 w-full">
        <Button
          ...
          disabled={loading}
        >
          {loading ? "Memproses..." : "Setujui & Proses"}
        </Button>
        <Button
          variant="outline"
