...
export const FinancialFailedView: React.FC = () => {
  const { withdrawal, setVariant, onFail, loading } = useFinancialModal();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!reason.trim()) {
      setError("Alasan kegagalan transfer wajib diisi");
      return;
    }
    setError(null);
    onFail(reason);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3.5 py-1 text-center flex flex-col items-center">
      {/* Destructive red failed indicator */}
      ...
      <div className="space-y-0.5">
        <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">
          Tandai Transfer Gagal
        </h3>
        <p className="text-xs text-muted-foreground max-w-[320px] mx-auto leading-relaxed">
          Sistem transfer gagal. Saldo akan dikembalikan penuh ke pengguna.
        </p>
      </div>
      ...
      {/* Input Textarea Alasan Gagal */}
      <div className="w-full space-y-1 text-left">
        <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-1">
          Alasan Kegagalan Transfer <span className="text-red-500 font-bold">*</span>
        </label>
        <Textarea
          placeholder="Contoh: Kode bank tidak terdaftar atau sistem kliring bank sedang bermasalah."
          value={reason}
          onChange={(e) => {
      ...
      <div className="flex flex-col sm:flex-row gap-2 pt-2 border-t border-slate-100 dark:border-slate-800 w-full">
        <Button
          ...
          disabled={loading}
        >
          {loading ? "Memproses..." : "Tandai Gagal"}
        </Button>
        <Button
          type="button"
          variant="outline"
