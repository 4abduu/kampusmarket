...
export default function LandingHeroSection({ featuredProducts, onNavigate, }: LandingHeroSectionProps) {
  const productsToShow: Product[] = (featuredProducts && featuredProducts.length > 0) ? featuredProducts : mockProducts.slice(0, 3);
    ...
    <section className="relative overflow-hidden bg-gradient-to-br from-primary-50 via-white to-secondary-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800">
      ...
      <div className="container mx-auto px-4 py-16 lg:py-24">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          {/* RIGHT - show fetched products if available, otherwise show placeholders from mock data (no full skeleton) */}
            ...
            <div className="relative grid grid-cols-2 gap-4">
              {productsToShow.map((product, index) => (
                <Card
                  ...
                  onClick={() => product.id && onNavigate("product", product.id)}
                >
                  ...
                  <CardContent className="p-3">
                    ...
                    <p className="text-primary-600 font-bold text-sm">Rp {Number(product.price ?? 0).toLocaleString("id-ID")}</p>
                  </CardContent>
                </Card>
              ))}
            </div>
