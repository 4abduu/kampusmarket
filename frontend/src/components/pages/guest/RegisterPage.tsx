...
import { Package, Mail, Lock, User, Eye, EyeOff, Phone, Building2, AlertCircle, Loader2, Check, TriangleAlert, ChevronDown } from "lucide-react";
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="email">
                ...
                <span className="text-amber-600 ml-1 text-xs inline-flex items-center gap-1 font-medium">
                  <TriangleAlert className="h-3.5 w-3.5" /> Permanen!
                </span>              
              </Label>
              <div className="relative">
            ...
            <div className="space-y-2">
              <Label htmlFor="faculty_id" className="flex items-center gap-1">
                <span>Fakultas <span className="text-red-500">*</span></span>
                <span className="text-amber-600 dark:text-amber-400 text-xs inline-flex items-center gap-0.5 font-medium ml-1">
                  <TriangleAlert className="h-3.5 w-3.5" /> Permanen!
                </span>
              </Label>
              ...
              <div className="relative">
                {isLoadingFaculties ? (
                  <div className="flex h-10 w-full items-center justify-center rounded-md border border-input bg-background">
                    ...
                    <span className="text-sm text-muted-foreground">Memuat fakultas...</span>
                  </div>
                ) : (
                  <>
                    ...
                    <select
                      ...
                      className={`flex h-10 w-full rounded-md border ${!formData.faculty_id && error ? "border-destructive" : "border-input"} bg-background pl-10 pr-4 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:opacity-50 appearance-none`}
              ...
              {/* Alert Box diselaraskan dengan gaya Amber/Warning agar senada dengan Email */}
              <div className="flex items-start gap-2 p-2 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/50 rounded-lg text-xs text-amber-700 dark:text-amber-400">
                <AlertCircle className="h-4 w-4 shrink-0 mt-0.5" />
                <span>
                  Pilihan fakultas bersifat permanen dan <strong>tidak dapat diubah</strong> setelah pendaftaran. Pastikan pilihan Anda sudah benar.
                </span>
              </div>
            </div>
            ...
            <Button type="submit" className="w-full bg-primary-600 hover:bg-primary-700" disabled={!formData.agreeTerms || !formData.faculty_id || isLoadingFaculties || isLoading}>
              {isLoading ? (
