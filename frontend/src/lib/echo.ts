...
/**
 * echo.ts — Pusher (menggantikan Reverb)
 *
 * Migrasi dari Reverb ke Pusher karena Railway tidak support
 * long-running WebSocket process di free plan.
 *
 * Setup .env frontend:
 *   VITE_PUSHER_APP_KEY=xxxxxxx
...
/**
 * Custom authorizer untuk private/presence channels.
 * Pakai apiClient (axios) biar cookie + Authorization header ikut terkirim.
 */
function makeAuthorizer(channel: Channel) {
  return {
    authorize(socketId: string, callback: (error: boolean, data: unknown) => void) {
...
export function getEcho(): Echo<'pusher'> {
  ...
  if (!key) {
    throw new Error('VITE_PUSHER_APP_KEY tidak ada di .env — Pusher tidak dikonfigurasi');
  }

  const cluster = (import.meta.env.VITE_PUSHER_APP_CLUSTER as string) || 'ap3';
  ...
  echoInstance = new Echo({
    ...
    cluster,
    forceTLS: true,
    // @ts-expect-error Callback signature mismatch between Echo and Pusher.js types
    authorizer: makeAuthorizer,
  });
